...
try:
    from troposphere import GetAtt, Join, Sub, Output, FindInMap
    from troposphere import Parameter, Ref, Template, ImportValue
    from troposphere.cloudfront import Distribution, DistributionConfig
    from troposphere.cloudfront import Origin, DefaultCacheBehavior
    from troposphere.cloudfront import ForwardedValues, ViewerCertificate
    from troposphere.cloudfront import CustomOrigin, Logging, CustomErrorResponse
...
domain_stack_name = t.add_parameter(Parameter(
    'DomainStackName',
...
# S3 Buckets
root_bucket = t.add_resource(Bucket(
    'RootBucket',
...
redirect_bucket = t.add_resource(Bucket(
    'RedirectBucket',
    BucketName=Sub('${ZoneName}-redirect'),
    AccessControl=PublicRead,
    WebsiteConfiguration=WebsiteConfiguration(
        RedirectAllRequestsTo=RedirectAllRequestsTo(
            HostName=Ref(zone_name),
            Protocol='https'
        ),
    )
))
...
log_bucket = t.add_resource(Bucket(
    'AccessLogBucket',
    BucketName=Sub('${ZoneName}-log')
))


# Cloudfront Distributions
root_distribution = t.add_resource(Distribution(
    'RootDistribution',
    DistributionConfig=DistributionConfig(
        ...
        Origins=[
            Origin(
                ...
                CustomOriginConfig=CustomOrigin(OriginProtocolPolicy='http-only')
            )
        ],
        ...
        Enabled=True,
        Logging=Logging(
            Bucket=GetAtt(log_bucket, 'DomainName'),
            IncludeCookies=True
        ),
...
# Route53 Records
root_alias_record = t.add_resource(RecordSetType(
    'RootDistributionAliasRecord',
    HostedZoneId=ImportValue(
        Sub('${DomainStackName}-R53Zone')
    ),
...
wildcard_alias_record = t.add_resource(RecordSetType(
...
t.add_output([
    ...
    Output(
        ...
        Value=Join('', ['http://', GetAtt(root_distribution, 'DomainName')])),
    Output(
    ...
    Output(
        ...
        Value=Join('', ['http://', GetAtt(redirect_distribution, 'DomainName')])),
    Output(
        'AccessLogBucket',
